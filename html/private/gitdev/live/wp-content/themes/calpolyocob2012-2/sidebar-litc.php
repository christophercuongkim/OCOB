...
<?php /* if($post->post_parent || count(get_pages('child_of='.$post->ID)) > 0) {
	if(trim(wp_list_pages( array('title_li'=>'','depth'=>2, 'echo' => '0', 'child_of'=>get_post_top_ancestor_id())) != "")){
	?>
    <h2>
    <?php 
    if($post->post_parent) {
...
<h2><a href="<?php echo "/litc"; ?>"><?php echo "CAL POLY LOW INCOME TAXPAYER CLINIC (LITC)"; ?></a></h2>

<?php wp_nav_menu(array('menu' => 'LITC')); //Grabs the Menu from the Appearance > Menu option in Wordpress?> 
