...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			parseDirection: 'y', //which direction to parse the table data
...
function makeRowPie()
{
    ...
    for (var i = 1; i < document.getElementById('t0').rows.length; i++) 
    {

        var thead = document.getElementById('t0').rows[0].cloneNode(true);
        var tr = document.getElementById('t0').rows[i].cloneNode(true);
        var tb = document.createElement("table");
        tb.setAttribute("id", "t" + i);
        tb.setAttribute("style", "display: none;");

        tb.appendChild(thead);
        tb.appendChild(tr);
        var pietitle = document.getElementById('t0').rows[i].cells[0].innerHTML;
        body.appendChild(tb);
        $('#t' + i).visualize({type: 'pie', pieMargin: 25, title: pietitle}).appendTo('body').trigger('visualizeRefresh');
    }
};
