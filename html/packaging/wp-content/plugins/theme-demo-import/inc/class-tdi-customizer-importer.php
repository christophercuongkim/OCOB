	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new WP_Error(
				...
				sprintf(
					esc_html__( 'The customizer import file is missing! File path: %s', 'theme-demo-import' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw  = TDI_Helpers::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'theme-demo-import' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'theme-demo-import' )
			);
		}

		// Import images.
		if ( apply_filters( 'theme-demo-import/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			require TDI_PATH . 'inc/class-tdi-customizer-option.php';

			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new TDI_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
