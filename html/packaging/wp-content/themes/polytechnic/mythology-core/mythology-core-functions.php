...
function mythology_localization() {
	// Old languages location
	load_theme_textdomain('mythology', get_template_directory() . '/theme-core/theme-functions/languages');
	$locale2 = get_locale();
	$locale_file2 = get_template_directory()."/theme-core/theme-functions/languages/$locale2.php";

	// New languages location
	load_theme_textdomain('mythology', get_template_directory() . '/languages');
	$locale = get_locale();
	$locale_file = get_template_directory()."/languages/$locale.php";

	if ( is_readable($locale_file2) ):
		require_once($locale_file2);
...
/**
* CROSS-COURSE NAV
* Allows for NEXT/PREV post navigation in courses.
*/
if ( ! function_exists( 'mythology_course_nav' ) ) :
/**
 * Display navigation to next/previous pages when applicable.
 */
function mythology_course_nav( $nav_id ) {
	global $wp_query, $post, $loop;
	...
	<?php if ( is_single() ) : // navigation links for single courses ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous course link', 'mythology' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next course link', 'mythology' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_page_template( 'template-course-catalog.php' ) || is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> More Related Courses', 'mythology' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'More Related Courses <span class="meta-nav">&rarr;</span>', 'mythology' ) ); ?></div>
		<?php endif; ?>
