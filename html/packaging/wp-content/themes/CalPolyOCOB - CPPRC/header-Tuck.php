...
<title>
<?php
if(get_field('website_title')!= null){
	echo get_field('website_title');
} else{
	bloginfo('name');  echo ' &mdash; Cal Poly, San Luis Obispo';
}
?>
</title>
...
<script src="https://webresource.its.calpoly.edu/jquery/1.10.2/jquery-1.10.2.min.js"></script>
...
<script>
/*smooth scroll fix for homepage*/

<?php 
$home_page_id = 2;
$is_home_page = false;
if($home_page_id == get_the_ID()){
$is_home_page = true;	
}
?>

var isHomePage = '<?php echo $is_home_page?>';
if (isHomePage == 1){
...
if (document.documentElement.clientWidth <= 760 && isHomePage == 1 ) {
	//do not load smooth scroll if homepage and small screen
} else {
	document.write('<script type="text/javascript" src="http://www.cob.calpoly.edu/gradbusiness/wp-content/themes/calpolyocobGrad/js/smoothscroll.min.js"><\/script>');
}
/*if going from desktop to small screen or visa versa and on homepage, refresh page*/
var currentWindowSize = document.documentElement.clientWidth;
var newWindowSize = 0;
window.onresize = function(){ 
newWindowSize = document.documentElement.clientWidth;
if(newWindowSize <= 760 && currentWindowSize > 760 && isHomePage == 1 ) {
	location.reload(); 	
} else if ( newWindowSize > 760 && currentWindowSize <= 760 && isHomePage == 1 ) {
	location.reload(); 	
}

}
</script>
<script src="http://www.cob.calpoly.edu/wp-includes/js/jquery/jquery-migrate.js?ver=1.2.1"></script>
...
?>
    ...
    <div id="headerContent">
        <div id="cplogo" style="margin-top:20px;margin-bottom:20px;"> <a href="<?php echo 'http://www.calpoly.edu';//get_bloginfo('url'); ?>" ><img src="https://webresource.its.calpoly.edu/cpwebtemplate/5.0.1/common/images_html/header/cp_logo_lrg.png" alt="Cal Poly, San Luis Obispo" id="thecplogo" title="" /></a> </div> <!-- cplogo -->
        <div id="deptName">
            <?php
    ...
    <li id="searchli">
        ...
        <form method = "get" title = "Search Form" action="http://search.calstate.edu/search">
          <div>
            ...
            <input type="text" id="q" name="q" placehlder="Search" title="Search this site" alt="Search Text" maxlength="256"/>
            <input type="image" id="searchSubmit" name="submit" src="http://webresource.its.calpoly.edu/cpwebtemplate/5.0.0/common/images_html/header/search-arrow.png" alt="Go" title="Submit Search Query" />
          </div>
        </form> -->
            </div>
