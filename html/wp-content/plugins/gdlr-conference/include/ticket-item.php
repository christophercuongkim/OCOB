	...
	*/	
	...
	function gdlr_print_ticket_item( $settings ){
		...
		while($query->have_posts()){ $query->the_post();
			...
			echo '<div class="gdlr-ticket-head-price">';
			if( empty($post_options['price']) ){
				echo __('Free', 'gdlr-conference');
			}else{
				echo gdlr_ticket_money_format($post_options['price']);
			}
			echo '</div>';
