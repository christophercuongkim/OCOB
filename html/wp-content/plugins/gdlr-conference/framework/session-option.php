<?php
	...
	*/	
	...
	function gdlr_register_session_item( $page_builder = array() ){
		...
		$page_builder['content-item']['options']['session-category'] = array(
			'title'=> __('Session Category', 'gdlr-conference'), 
			'type'=>'item',
			'options'=>array_merge(gdlr_page_builder_title_option(true), array(					
				'category'=> array(
					'title'=> __('Parent Category' ,'gdlr-conference'),
					'type'=> 'combobox',
					'options'=> gdlr_get_term_list('session_category'),
					'description'=> __('Nothing will show up if the selected category has no child categories. Date will not be displayed for each session as well.', 'gdlr-conference')
				),		
				'session-style'=> array(
					'title'=> __('Item Style' ,'gdlr-conference'),
					'type'=> 'combobox',
